...
export const getBooksByLanguage: RequestHandler = async (req, res, next) => {
  ...
  try {
    const langRes = await query('SELECT * FROM languages WHERE id = $1', [languageId]);
...
export const getEditForm: RequestHandler = async (req, res, next) => {
  ...
  try {
    const languageRes = await query('SELECT * FROM languages WHERE id = $1', [languageId]);
...
export const editLanguageById: RequestHandler = async (req, res, next) => {
  ...
  try {
    const { rowCount } = await query('UPDATE languages SET name = $1 WHERE id = $2', [
      name,
      languageId,
    ]);
...
export const deleteLanguageById: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    const bookRes = await query('SELECT 1 FROM book_languages WHERE language_id = $1 LIMIT 1', [
      languageId,
    ]);
    ...
    const languageRes = await query('DELETE FROM languages WHERE id = $1', [languageId]);
    if (languageRes.rowCount === 0) {
      throw new CustomNotFoundError('Language Not Found');
    }
